...
public class Filters {
    ...
    public static boolean FiltroPosterize(BufferedImage image,String nomefilearrivo,int intervallo)throws Exception{
        for (int col = 0; col < image.getWidth(); col++) {
            for (int row = 0; row < image.getHeight(); row++) {
                Color pixelInput = new Color(image.getRGB(col,row));
                Color pixelPosterize= new Color(posterize(pixelInput.getRed(),intervallo) ,posterize(pixelInput.getGreen(),intervallo),posterize(pixelInput.getBlue(),intervallo));
